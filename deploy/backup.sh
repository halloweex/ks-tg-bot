...
# --- failure alerting -------------------------------------------------------
# notify(), alert_chat_id() and _where() live in deploy/notify.sh because
# restore-test.sh needs the same channel for the same reason — that file also
# explains why this is Telegram and not mail. Alerting runs outside the logged
# section below, so the token never reaches the log file.
