...
def _back_to_info_kb(t: Texts, instagram_url: str = "") -> InlineKeyboardMarkup:
    """Back to the page list, and — on contacts — the way to Instagram.

    The Instagram button wears Instagram's own logo rather than a camera
    (icon_custom_emoji_id), which is the same permission and the same fallback
    as the logos in the order blocks: a refused icon costs the icon.
    """
    builder = InlineKeyboardBuilder()
    if instagram_url:
        builder.button(text=t.BTN_INSTAGRAM, url=instagram_url,
                       icon_custom_emoji_id=texts.INSTAGRAM)
...
async def _show_info_page(callback: CallbackQuery, text: str, t: Texts,
                          page: str = "", instagram_url: str = "") -> None:
