...
def _order(order_id: int = 900001, *, phone: str = PHONE, **kw) -> Order:
    return Order(
        source="keycrm", source_order_id=str(order_id), buyer_phone=phone,
        status_name="delivered", status_group_id=1, grand_total=1450.0,
        currency="грн", ordered_at="2026-07-14T09:12:33", **kw,
    )
...
def test_an_order_for_a_stranger_costs_no_write(db):
    ...
    _register()
    result = asyncio.run(
        sync_changed_orders(FakeCRM([_order(phone="+380990000000")]), now=NOW)
    )
