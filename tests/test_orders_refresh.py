...
def called(monkeypatch):
    ...
    async def fake_sync(chat_id: int, phone: str, keycrm, shopify) -> None:
        calls.append((chat_id, phone, keycrm, shopify))
...
def test_the_refresh_reaches_the_scenario(called):
    ...
    keycrm, shopify = object(), object()
    asyncio.run(handler._refresh_orders(CHAT, keycrm, shopify))
    assert called == [(CHAT, PHONE, keycrm, shopify)]
...
def test_a_user_without_a_phone_is_never_synced(called, monkeypatch):
    ...
    asyncio.run(handler._refresh_orders(CHAT, object(), None))
    assert called == []
