...
### Перевод экранов на блоки закончен на двух, работа ушла в то, что сломано

Аудит пяти оставшихся плоских экранов дал один ответ на все пять: **не
мигрировать.** Либо блоки там ничего не дают (меню из трёх слов, настройки из
одной строки), либо мешают: тексты Довідки пишет владелица в HTML в
`config.yaml`, а текст блока Telegram не парсит — и две страницы из четырёх
несут `<tg-emoji>`, который в блоках небезопасен. То есть выигрыш и блокер там
совпадают в одном месте. Экран Доставки вообще оказался **недостижим**: ни одна
клавиатура его не рисует.

«Два экрана из семи» — законное конечное состояние, а не недоделка: правило
проекта «экран мигрирует всеми входами разом», а не «все экраны мигрируют».

Зато аудит нашёл то, что у покупательницы сломано сегодня:

- **С экрана выбора языка нельзя было выйти.** Единственная инлайн-клавиатура
  в боте без пути назад; при `bottom_menu: false` выхода нет вообще. Гард-тест
  на тупики существовал и не поймал — `language_kb` просто не была в его списке.
- **Номер, отправленный «в никуда», уходил в тишину.** Клавиатуру «Поділитися
  номером» нельзя снять правкой сообщения, поэтому после «📋 Меню» она висит; тап
  по ней слал контакт, который не ловил никто — оба обработчика под фильтром
  состояния. Теперь номер сохраняется, а `ReplyKeyboardRemove` наконец убирает
  зависшую клавиатуру. Проверку владения не дублировал: `own_contact_phone`
  уже возвращает None для чужой визитки.
- **Тап «Де посилка?» сворачивал то, что она читала.** `is_open` и `parcels`
  были одним выражением, поэтому ответ по третьему заказу закрывал первый.
- **Вторая «📋 Меню»** на rich-экране заказов — третье появление того же
  класса; правило теперь общее: ни одна кнопка не может быть и в слэбе, и в
  блоках.
- **Детектор миграции был заглушён заранее.** `plain_ok=True` стоял в
  двенадцати местах и заработан в одном — проверено перечислением callback,
  которые вообще существуют на rich-экранах (их два).

### CI поймал сюиту с первой же попытки

В деплое не было ни `pytest`, ни `lint-imports`, а `config.yaml` уезжал в прод
без единой проверки — `load_config()` зовётся внутри `main()`, поэтому шаг
«импортируется точка входа» конфиг не читает. Добавил и то и другое.

Первое, что поймал новый CI, — **саму сюиту**. Три теста могли пройти только на
машине разработчика: один запускал `.venv/bin/alembic` (путь, а не программу),
два строили `EnvSettings()` голым и занимали токены из лежащего рядом `.env`.
«Тесты зелёные» всё это время означало «зелёные на одной машине». Проверено
прогоном копии дерева без `.env`: 903 там и 903 здесь.

Плюс проверка HTML в текстах владелицы: чужой тег, незакрытый тег и пустой
`<tg-emoji>` (у которого нечему стать заменой, когда кончится Premium).

### Маршрут вернули, и поставили сторожа на тишину

Маршрут `/rivo/` восстановлен в конфиге соседнего проекта с разрешения
владельца: блок по их конвенциям, `nginx -t`, копия рядом. Снаружи вебхук
теперь отвечает `401 bad signature` от нашего бота вместо `405` от чужого
FastAPI; соседний сайт не задет.

По дороге напоролись на ловушку, которую стоит запомнить: `nginx.conf`
смонтирован как **файл**, а bind-mount файла привязан к inode. Замена файла
целиком разрывает связь — контейнер читает старый inode, а `nginx -t` и
`nginx -s reload` честно подтверждают **старый** конфиг. Применяется только
пересозданием контейнера.

И сразу выяснилось, что соседний проект наступил на это сам 6 сентября: их
контейнер поднялся в 20:12, файл переписали в 20:16, и пять суток работал
конфиг, которого нет на диске. По счастью, различие оказалось одним
комментарием.

**Сторож.** Канал был мёртв одиннадцать дней, и заметить это было нечем:
эндпоинт отвечал, контейнер здоров, деплои зелёные, а каждый запрос уходил в
чужое приложение. Поэтому следим не за ошибкой — её нет, — а за **тишиной**,
как `bot/sync.py` следит за развёрткой заказов. Нет обращений трое суток —
предупреждение админам, с двумя вещами к проверке по порядку.

Приход отмечается **до** проверки подписи: поддельный запрос тоже доказывает,
что путь из интернета достаёт до процесса, а считать только подписанные значило
бы смешать «никто не звонит» с «звонят, но ключ сменился». Мутация показала, что
без отдельного теста этот инвариант ничем не закреплён.

Тишина меряется от более позднего из двух: последнего прихода и момента старта
процесса — иначе свежий деплой сразу предупреждает о неделе, когда он не
работал, и так предупреждения заглушают.

### Фикстуры Rivo: их нельзя достать, и причина — мёртвый канал

Просьба была достать фикстуры. Достать нечего: **вебхук лояльности не работает
с 6 сентября.** Маршрут `/rivo/` исчез из конфига nginx соседнего проекта, когда
тот переписали, — запрос снаружи получает `405` от чужого FastAPI, до бота не
доходя. Проверено `nginx -T`, датой файла и живым запросом по настоящему адресу.

Сверх того: в логах nginx за пять суток ни одного обращения к `/rivo/`, а в
`outbox` (шапки живут год) — ноль сообщений лояльности и рефералов за всю
историю. Похоже, Rivo не звонит вовсе, но это видно только из его кабинета.

Восстановление маршрута — правка в чужом проекте, самовольно не сделана.

**Что сделано вместо.** Сборщик фикстур: тело, которое бот не смог полностью
разобрать, записывается само — по одному файлу на тип события, без перезаписи,
с вычищенными персональными данными. `customer` пересобирается из тех двух
полей, которые читает парсер, а не фильтруется по списку запрещённых: список
защищает только от полей, о которых подумали.

Ключевая тонкость: пробу берёт не «`parse_event` вернул None», а отдельный
предикат в самом парсере. Форма `points_diff: null` — та, из-за которой
покупательница не узнала о начислении, — разбирается в **исправное** событие,
которое потом отбрасывает `announce`. Сборщик, следящий только за None, не
увидел бы именно её.

Проба берётся строго после проверки подписи: иначе эндпоинт стал бы способом
класть файлы на диск постороннему.

### Атрибуция кампаний: пункт 19, и он оказался не про проводку

§6.4 просил воронку «отправлено → нажато → куплено» и ключ кампании в
`callback_data` кнопки. Половина этого никогда не была осуществима: **ни одно
проактивное сообщение не несёт callback-кнопки.** Три вида без клавиатуры вовсе,
два с `url` (про который Telegram боту не сообщает ничего — это `/stats` писал
про кнопку «Сайт» задолго до §6.4), шестое открывает инлайн-панель запросом,
который видно в поле ввода покупательницы. Штамповать ключ было некуда — отсюда
и `parse()` без вызывающих.

Купить середину можно только поставив кнопку на сообщение, у которого её нет, и
направив её обратно в бота вместо магазина. Это продуктовое изменение ради
числа, и оно отклонено.

Вместо середины — обе честные половины, которые уже лежали на диске: `outbox`
хранит шапку с ключом год, `orders` заполняется синхронизацией без участия
покупательницы. Отчёт в `/stats`, ноль изменений в клиентском опыте, `parse()`
получил первого настоящего вызывающего, а мёртвый `campaign_key` из payload
транспорта удалён — он писался для читателя, которого не существует.

Отчёт отказывается врать в шести местах: прочерк вместо нуля там, где нет
наблюдения; никакой выручки; поддержка не кампания; отчёт админу не получатель
(он под тем же ключом, что рассылка — отсюда группировка по паре «ключ, тип»);
никаких процентов на выборке меньше десяти; и никакого фонового окна «до» для
кампаний, которые сама покупка и вызывает.

И один символ, который сделал бы всё это ложью: `'T'` в `ordered_at` против
пробела в `sent_at` — при текстовом сравнении заказ, сделанный за час **до**
сообщения, читается как сделанный после. Проверено запуском, закрыто
`datetime()` с обеих сторон, закреплено тестом на формате, снятом с боевой базы.
