...
|---|---|
| 4.1 `ERR_INVALID_PHONE` требует запрещённого | закрыт: текст больше не просит набирать руками, у экрана есть выход к менеджеру |
...
| 4.6 Поддержка здоровается каждый раз | закрыт: приветствие ушло вместе с голосом бренда — «Напиши свій запит» |
| 4.7 Мёртвый код и документация | закрыт: `MSG_LANGUAGE_CURRENT` удалён, докстринг переписан; `delivery.py` осознанно осиротел |
| 4.8 Кнопка скидки не смотрит, под чем стоит | закрыт: несёт sku, спрашивает про конкретный товар |
| 4.9 Мелочи | частично: у заказа появились действия; нумерацию посылок не делаем — экран осиротел, ссылаться на номера неоткуда |
...
скидок нет (`open-decisions`, пункт про два кода скидок в Shopify), и кнопка
сознательно лишь копит заявки.
...
> **Статус 2026-08-25.** Все пять шагов сделаны. Ниже — исходный план, он же
> объяснение решений. Чего в нём не было и что появилось сверху: инлайн-панели,
> второй вид меню, дайджест заказов, дни рождения, голос бренда на «ти»
> (`CHANGELOG.md`, 2026-08-25).
>
> Последним закрылся пункт 1 шага 1, и не так, как здесь написано: клавиатуры
> мало. Ключи меню отключены на время шаринга телефона (`menu.py:53`) — и это
> правильно для всех остальных ключей, — поэтому «💬 Менеджер» обработан внутри
> состояния, выше catch-all, в онбординге и в смене номера. Обычная клавиатура
> шаринга осталась с одним ключом: выход появляется только после осечки.
...
- **Deep links** на конкретный заказ — единственный непостроенный пункт
  ...
  или Shopify (`open-decisions`, пункт про ссылку на бота в письме о заказе).
  Пока в боте 12 зарегистрированных (проверено
  по базе 2026-08-25; десять из них пришли 24–25 августа) при ~19 500 клиентах
  в CRM, любая работа над экранами имеет меньший эффект, чем одна ссылка в
  письме.
...
- выход к менеджеру с экрана шаринга: поделиться контактом, номер которого не
  разбирается, — и убедиться, что второй ключ появился и уводит в поддержку
  (проверяется только вживую: клавиатуру рисует клиент);
