...
**Отношение к §5.4 и §5.5 документа.** v3 уже решает половину этой задачи:
...
**12.8. Четыре подсистемы — контракт есть, этапов в плане нет.**
`analytics`, `stock`, `discount_requests`, `demo` получили зону ответственности,
интерфейс и инварианты в разделе 10, а не только адрес в карте переезда. Пятая,
`i18n`, документом покрыта — §7 требует свести тексты бота и веба в один модуль
`core`.

Что этим **не** решено: в `.planning/research/ARCHITECTURE.md` этих четырёх нет
ни строкой (проверено грепом по v3: «аналитик», «сток», «остатк», «скидк»,
«демо», «demo» — ноль совпадений), поэтому §12 того документа не содержит для
них ни одного этапа и ни одной оценки. Двое из четырёх на критическом пути:
`stock` — первый отправитель через outbox (§10.2), то есть часть этапа 6;
`analytics` — источник критерия готовности «видна конверсия» сразу для этапов 7
и 8. Правок в `.planning/research/ARCHITECTURE.md` этот документ не делает — это
его расхождение, не наше.
