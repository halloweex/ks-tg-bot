...
Целевое состояние. Решения `.planning/research/ARCHITECTURE.md` (Postgres, KeyCRM как
...
прямо противоречит §4.1 документа («три образа гарантированно приведут к двум разным
...
все три точки входа; тексты, по §9 документа, должны быть сведены в один модуль
...
class AmbiguityPolicy(StrEnum):     # §6.2 документа
...
def expected_runout(last_purchase: date, prior_days: int, personal_gaps: Sequence[int]) -> date: ...
    # смешивание с весом n / (n + 2), §8.2 документа
```
...
class SqlOutboxRepo:
    ...
    # Транзакция закрывается ДО отправки, §6.3 документа.
```
...
class RegisterUser:
    ...
    # link_orphans вызывается с ограничением по давности, §5.4 документа
...
3. **Курсор синка двигается только после успешной записи всей страницы**, окно
   берётся с перехлёстом (§5.2 документа).
...
нельзя протестировать без сети, и утверждение §4.1 документа «бот в сеть за данными
...
2. В URL нет внутренних числовых идентификаторов — номер заказа, который клиент
   знает (§4.7 документа).
3. Пул соединений — свой на процесс, pgbouncer не ставится (§12 документа).
...
5. Каждая джоба пишет `last_success_at`; алерт строится на отсутствии успеха, а
   не на наличии ошибки (§5.5 документа).

---

## 10. Подсистемы, которых нет в архитектурном документе

Шесть подсистем работают в коде и не упомянуты в `.planning/research/ARCHITECTURE.md` ни разу
(шестая, §10.6, появилась позже самого документа)
...
**И, с 16.09.2026, не только продуктовые.** В той же таблице живут три
служебных момента сторожа вебхука лояльности — `rivo_webhook_arrived`,
`rivo_watch_started`, `rivo_silence_alerted` (`bot/webhooks.py`), с `chat_id`
NULL. Не из лени: сторожу нужен ровно один вопрос («когда это было в последний
раз»), на который `events` уже отвечает через `last_seen`, таблица append-only и
не подрезается, а своя таблица на три строки в год потребовала бы миграции и
второго способа писать время. На отчёты они не влияют — все агрегаты либо
группируют по имени события, либо фильтруют по нему, либо отбрасывают строки без
пользователя; видимый след — две лишние строки в блоке Events у `/stats`.
...
**Отношение к §6.4 и §6.4 документа.** v3 уже решает половину этой задачи:
прунинг оставляет шапку на год (§5.5), а формат `dedup_key` несёт измерения
атрибуции (§5.4, `user:{id}:replenish:sku:{sku}`). Это закрывает сторону
«отправлено» — она переживает чистку.

Не закрыта вторая половина: **сторона «нажато»**. Ключ живёт в базе, а не в
сообщении, поэтому связать нажатие кнопки с кампанией нечем — `events` увидит
`orders_viewed`, но не то, откуда человек пришёл. Отсюда требование выше: тот же
ключ кладётся в `callback_data` каждой кнопки проактивного сообщения. Стоит это
одного поля, добавляется только до первой отправки, и без него воронка остаётся
двухточечной — отправлено и куплено, без середины.
...
### 10.6. Род обращения → `core.usecases.gender` + `core.repos.buyer_gender`

Подробно — `docs/gender.md`; здесь только контракт слоёв.

**Зона ответственности.** Узнать у хранилища, какого рода покупатель, свести его
покупателей CRM с нашими чатами и положить ответ туда, где его читает
middleware. Само решение принимается вне этого репозитория.

```python
# core/domain/gender.py  (чистое)
class Gender(str, Enum): F = "f"; M = "m"; UNKNOWN = "u"
DEFAULT = Gender.F
def form(stored: str | None) -> Gender: ...
def resolve(genders: Iterable[str | None]) -> Gender: ...

# core/ports/gender.py
...
# core/usecases/gender.py
async def refresh_forms(directory, source, forms, *, only=None) -> dict[int, str]: ...
```

**Инварианты.**

1. **Единственное место, где живёт чужой драйвер, — `core/repos/buyer_gender.py`**,
   по контракту `only-repos-touch-db`. Это чужая база: читается read-only, ролью
   `ks_readonly`, и ни одного write в ней не появляется. Таблица не наша, адрес
   её — конфиг, а не константа в коде.
2. **Форма не решается ниже точки входа** — тот же инвариант, что у языка в
   §10.4, и по той же причине. `Texts` собирается в middleware и приходит вниз
   готовым; хендлер, который сам читал бы колонку, это хендлер, который может
   забыть.
3. **`core.i18n` не знает `core.domain`** (`i18n-is-a-leaf`), поэтому три буквы
   написаны дважды: значения `Gender` и ключи таблиц перекрытий. Держит их вместе
   тест, а не комментарий, — `tests/test_gender_texts.py`.
4. **Пустая колонка — это женская форма, а не «без рода».** Инвариант, а не
   умолчание: он гарантирует, что отказ чужой базы или ненастроенный DSN не
   меняют голос бренда для всей базы. Различить «не спрашивали» и «спросили и не
   знаем» умеет только `'u'`.
5. **Чужая база не появляется на пути апдейта.** Спрашивают свип раз в час и
   регистрация; экран читает свою колонку тем же запросом, которым читает язык.
...
контракт §6.3 документа существует только в комментарии к SQL.
...
**12.8. Подсистемы — закрыто в v4.**
`analytics`, `stock`, `discount_requests`, `i18n`, `demo` описаны в v4 §7, вместе
с правилом изоляции демо и ролью стока как пилота outbox. Раздел 10 здесь и §7
там теперь говорят одно и то же; при расхождении верен документ.
...
## 13.5. Сверка с architecture.md v4

v4 объявлен финальным и вобрал модульный контракт целиком (§3), пять подсистем
(§7) и три невыразимых инварианта. Что из этого следует для документа, который
вы читаете.

**Ссылки перенумерованы.** v4 сменил структуру: v3 §3.x → v4 §4.x, §4.x → §5.x,
§5.x → §6.x, §6.x → §8.x, §7 → §9, §8 → §10, §9 → §11, §10 → §12, §12 → §14.
Все ссылки «§N документа» здесь и в `current-state.md` пересчитаны.

**Что v4 теперь владеет, а этот документ дублирует.** Раздел 1 («три модуля
сверх списка») больше не нужен как обоснование: v4 §3 принял `core.usecases`,
`core.config` и `core.i18n` без оговорок. Раздел 2 (стек слоёв) и правила
импортов повторяют v4 §3. При расхождении верен v4.

**Где расходится раскладка файлов.** v4 §3 кладёт `CampaignKey` в
`core/domain/campaign.py`, а тихие часы в `core/domain/quiet.py`; здесь и то и
другое живёт в `core/domain/outbox.py`. v4 кладёт демо в `bot/demo.py`, здесь —
`bot/handlers/demo.py`. Мелочь, но при переносе разойдётся, если не выбрать одно.

**Правило v4, которого нет в `.importlinter`:** «`core.domain` не импортирует
даже `core.config`». Добавлено контрактом `domain-knows-no-config`.

**Решения, которых не было в v4 при сборке — внесены в него.** Четыре пункта
(`fsm_state`, `support_albums`, частичный уникальный индекс заказов, `stock_levels`
вместо `stock_state`) были в коде и в этом документе, но не в архитектуре. Все
четыре добавлены в v4: §4.4, §4.6 и §6.7. Список оставлен здесь как след того,
чем расхождение было, а не как открытый вопрос.

---|---|---|
| `fsm_state` — состояние диалога в базе | в проде, миграция 5 | без него деплой оставлял клиента в тишине; в §4.6 таблицы нет |
| `support_albums` — заявка на альбом | в проде, миграция 6 | без него из трёх фото доходило одно; в §4.6 таблицы нет |
| Частичный уникальный индекс `WHERE merge_key != ''` | в проде, миграция 3 | полный индекс превращал откат образа в тихую потерю кеша заказов: измерено, три заказа на входе — одна строка на выходе. §4.4 про это не говорит |
| `stock_levels` | в проде | v4 §4.6 называет ту же таблицу `stock_state` |
