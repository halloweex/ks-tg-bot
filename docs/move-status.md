...
Шесть сценариев на портах — `analytics`, `sync_catalogue`, `support`, `notify`,
`birthdays`, `stock`. `core.repos` напрямую импортируют ещё пять: `referrals`,
`broadcast`, `sync_orders`, `register`, `sync_incremental`. Итого одиннадцать,
как и в абзаце про шим выше.
...
| 14 | **`referrals`** | да + семейное на `core.repos.referrals` |
...
коммите. Хендлеры в счёт не идут: правило про `core.usecases`. По плану остаются
`referrals` (14), `outbox` и `broadcast` (16), а `orders`, `users` и `sync_state`
освобождаются все разом на 21-м — после чего семейный контракт целиком заменяется
на `core.usecases` внутри `core-siblings-are-independent` (22).
